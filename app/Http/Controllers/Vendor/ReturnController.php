...
class ReturnController extends Controller
{
    ...
    {
        $products = Returnproduct::where('creator_role','vendor')->where('created_by',Auth::guard('vendor')->user()->id)->orderByDesc('id')->get();
        return view('vendor.return.index', compact('products'));
    }
    ...
    public function create()
    {
        ...
        return view('vendor.return.create', compact('products'));
    }
    ...
    public function store(Request $request)
    {
        ...
        $purchase = Purchase::where('creator_role','vendor')->where('product_id',$request->product_id)->where('created_by', Auth::guard('vendor')->user()->id)->orderByDesc('id')->first();
        ...
        if ($purchase != null) {
            if ($request->qty > $purchase->qty && $product->avilable_qty<$request->qty) {
                ...
                if ($result) {
                    ...
                    return redirect()->route('v_return.create');
                }
                else
                {
